...
module.exports = function(homebridge){
  ...
  homebridge.registerAccessory("homebridge-tesy-waterheater", "TesyWaterHeater", TesyWaterHeater);
};
...
class TesyWaterHeater {
  ...
  refreshTesyWaterHeaterStatus() {
    this.log.debug("Executing RefreshTesyWaterHeaterStatus from:", this.apiroute);
        ...
        exec(command3, function(error, stdout, stderr){
            ...
            stdout.split('\n').map(item => {
              if (item.toLowerCase().includes('heater_state')) {
                ...
                var newCurrentTemperature = parseFloat(response.cur_shower);
                ...
                var newHeatingThresholdTemperature = parseFloat(response.ref_shower);
                ...
                var newHeaterActiveStatus = that.getTesyWaterHeaterActiveState(response.power_sw);
                ...
                var newCurrentHeaterCoolerState = that.getTesyWaterHeaterCurrentHeaterCoolerState(response.heater_state);
      ...
      exec(command2, function(error, stdout, stderr){
        ...
        exec(command3, function(error, stdout, stderr){
            ...
            this.service.getCharacteristic(Characteristic.Active)
              .updateValue(that.getTesyWaterHeaterActiveState(response.power_sw));
          });
        });
      });
  }
    ...
    exec(command, function(error, stdout, stderr){
      ...
      exec(command2, function(error, stdout, stderr){
        ...
        exec(command3, function(error, stdout, stderr){
            ...
            var currentTemperature = parseFloat(response.cur_shower);

            that.log.debug("CurrentTemperature is: %s", currentTemperature);

            that.pullTimer.start();
            this.service.getCharacteristic(Characteristic.CurrentTemperature)
              .updateValue(that.getTesyWaterHeaterActiveState(currentTemperature));
        });
      });
    });
  }
  ...
  getServices() {
    ...
    this.refreshTesyWaterHeaterStatus();
